...
8. The discovery flow exports VNet flow logs from the selected Log Analytics workspace using KQL and analyzes internal traffic, north-south egress, and public inbound exposure for the covered VNets only. All KQL queries include a `top N` row limit to prevent timeouts in large environments.
